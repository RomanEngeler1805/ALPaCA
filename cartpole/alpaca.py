...
tf.flags.DEFINE_integer("update_freq_post", 5000, "Update frequency of posterior and sampling of new policy")
...
tf.flags.DEFINE_float("regularizer", 0.0005, "Regularization parameter")
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for episode in range(FLAGS.N_episodes):
        ...
        for e in range(batch_size):
            ...
            if len(valid)>0: # should not be empty
                if episode < 500:
                    valid = np.random.choice(valid, size=np.int(0.9*len(valid)), replace=False)
                elif episode < 1000:
                    valid = np.random.choice(valid, size=np.int(0.4*len(valid)), replace=False)
                else:
                    valid = np.random.choice(valid, size=np.int(0.1*len(valid)), replace=False)
        ...
        if episode % FLAGS.save_frequency == 0:
            ...
            for row, xpos in enumerate([2.4, 0, 2.4]):
                for col, xacc in enumerate([-max_thetadot, 0., max_thetadot]):
            ...
            # evaluate on 10 different initial positions
            # loop tasks --------------------------------------------------------------------
            ...
            for n in range(10):
                ...
                while step < FLAGS.L_episode:
                    # take a step
                    Qval = sess.run([QNet.Qmean], feed_dict={QNet.state: state.reshape(-1, FLAGS.state_space)}) # mean policy
                    action = eGreedyAction(Qval, eps)
                    next_state, reward, done, _ = env._step(action)
