...
def plot_Value_fcn(path, delta, sess, model, noise_precision, buffer=[]):
    ...
    if buffer:
        # fill arrays
        # [s, a, r, s', a*, d]
        state_train, action_train, reward_train = buffer

        # update
        sess.run(model.reset_post)
        wt_bar = sess.run([model.wt_bar],
               feed_dict={model.context_state: state_train, model.context_action: action_train,
                          model.context_reward: reward_train, model.nprec: noise_precision})
    ...
    for act in range(ncols):
        ...
        im = ax[act].contourf(mesh_pha, mesh_rad, Q_r, colormap="plasma")
