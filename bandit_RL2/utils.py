...
def minimize_and_clip(optimizer, objective, var_list, clip_val=10):
    ...
    for idx, (grad, var) in enumerate(gradients):
        if grad is not None:
            gradients[idx] = (tf.clip_by_norm(grad, clip_val), var)
    ...
    # concat summaries
    summaries_gradvar = tf.summary.merge([grad_summaries, weight_summary])


    return summaries_gradvar, optimizer.apply_gradients(gradients)
