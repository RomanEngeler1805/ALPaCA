...
tf.flags.DEFINE_integer("state_space", 12, "Dimensionality of state space")
...
tf.flags.DEFINE_integer("L_episode", 1000, "Length of episodes")
...
from plume_normal import PlumeEnv
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for episode in range(FLAGS.N_episodes):
        ...
        for n in range(FLAGS.N_tasks):
            ...
            state = np.concatenate([state[0:6], goal_state[0:6]])
            ...
            while step < FLAGS.L_episode:
                ...
                next_state = np.concatenate([next_state[0:6], goal_state[0:6]])
                reward
